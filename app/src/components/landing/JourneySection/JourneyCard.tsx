      ...
      <div className="w-full lg:w-1/2 flex flex-col justify-between space-y-6 relative z-10">
        <div>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-3xl font-black font-mono text-white/30 group-hover:text-white/60 transition-colors">
              {card.stepNumber} <span className="text-sm font-normal text-white/20">/ 0{totalCards}</span>
            </span>
          </div>
            ...
            <div className="grid grid-cols-2 gap-4 py-4 px-4 rounded-xl bg-slate-950/60 border border-white/5 mb-6">
              {card.stats.map((stat, sIdx) => (
                <div key={sIdx}>
                  ...
                  <div className="text-lg sm:text-xl font-bold text-white font-mono mt-0.5">{stat.value}</div>
                </div>
              ))}
            </div>
          )}
...
const CardVisualPreview = React.memo(function CardVisualPreview({ type }: { type: JourneyCardItem["previewType"] }) {
  switch (type) {
        ...
        <div className="w-full bg-slate-950/80 border border-white/10 rounded-2xl p-5 shadow-2xl space-y-4">
          <div className="flex items-center justify-between pb-3 border-b border-white/10">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-mono text-slate-400 ml-2">zyr0.co/internships</span>
            </div>
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={idx} className="p-3.5 rounded-xl bg-slate-900/90 border border-white/5 hover:border-cyan-500/30 transition-all flex items-center justify-between">
                <div>
                ...
                <span className="text-xs font-mono font-bold text-cyan-400">{drop.stipend}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="w-full bg-slate-950/80 border border-white/10 rounded-2xl p-5 shadow-2xl space-y-4">
          <div className="flex items-center gap-4 pb-4 border-b border-white/10">
            ...
            <div>
              ...
              <div className="text-xs text-slate-400">BS Computer Science • NUST 2026</div>
            </div>
          </div>
          ...
          <div className="pt-2 flex items-center justify-between bg-slate-900/90 p-3 rounded-xl border border-white/5">
            <div className="flex items-center gap-2 text-xs text-slate-300">
              <ShieldCheck className="w-4 h-4 text-emerald-400" /> Verified GitHub & Transcripts Attached
            </div>
    ...
    case "selection_pipeline":
      return (
        <div className="w-full bg-slate-950/80 border border-white/10 rounded-2xl p-5 shadow-2xl space-y-4">
          <div className="text-xs font-bold text-slate-400 tracking-wider uppercase mb-2">Live Application Tracker</div>
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={idx} className="flex items-center justify-between p-3 rounded-xl bg-slate-900/90 border border-white/5">
    ...
    case "workspace_sprint":
      return (
        <div className="w-full bg-slate-950/80 border border-white/10 rounded-2xl p-5 shadow-2xl space-y-4">
          <div className="flex items-center justify-between pb-3 border-b border-white/10">
            <div className="text-xs font-bold text-white flex items-center gap-2">
              <Sparkles className="w-4 h-4 text-amber-400" /> Milestone Workspace
            </div>
            <span className="text-xs font-mono text-amber-400 font-bold">Sprint 2 / 4</span>
          </div>

          <div className="p-3 rounded-xl bg-slate-900/90 border border-white/5 space-y-2">
            <div className="flex justify-between items-center text-xs">
              <span className="font-semibold text-white">Task: Implement OAuth2 Authentication</span>
            ...
            <p className="text-[11px] text-slate-400">Assigned Mentor: Tariq Mahmood (Senior Staff Engineer)</p>
          </div>

          <div className="space-y-2">
            <div className="flex items-center justify-between text-[11px] text-slate-400">
              <span>Rubric Check: Code Quality & Tests</span>
              <span className="text-emerald-400 font-bold">PASSED (10/10)</span>
            </div>
            <div className="flex items-center justify-between text-[11px] text-slate-400">
      ...
      return (
        <div className="w-full bg-slate-950/90 border border-purple-500/30 rounded-2xl p-6 shadow-2xl relative overflow-hidden space-y-4">
          ...
          <div className="space-y-1">
            <div className="text-lg font-extrabold text-white">Software Engineering Internship</div>
            <div className="text-xs text-slate-400">Issued to: Ayesha Khan • August 2026</div>
          </div>
        ...
        <div className="w-full bg-slate-950/80 border border-white/10 rounded-2xl p-5 shadow-2xl space-y-4">
          <div className="flex items-center justify-between pb-3 border-b border-white/10">
            <span className="text-xs font-bold text-white flex items-center gap-2">
              <Rocket className="w-4 h-4 text-blue-400" /> Full-Time Transition Pool
            </span>
            <span className="text-xs font-bold text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/30">HIRED</span>
          </div>

          <div className="p-4 rounded-xl bg-slate-900/90 border border-white/5 text-center space-y-2">
            <div className="w-10 h-10 rounded-full bg-blue-500/20 border border-blue-500/40 flex items-center justify-center text-blue-400 mx-auto">
              <Building2 className="w-5 h-5" />
            </div>
            <div className="text-sm font-bold text-white">Direct Return Offer Generated</div>
            <div className="text-xs text-slate-400">Systems Limited • Junior Software Engineer</div>
            <span className="inline-block mt-2 px-3 py-1 rounded-full text-xs font-bold bg-blue-600 text-white shadow-md">
              Offer Accepted
            </span>
          </div>
        </div>
