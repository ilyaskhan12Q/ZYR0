...
export async function decompose(topic: string, keys: ZeroAiKeys): Promise<DecompositionResult> {
  const started = performance.now();
  const res = await callLlm(SYSTEM_PROMPT, buildUserPrompt(topic), keys);
  const elapsedMs = Math.round(performance.now() - started);
  if (res.ok) {
    const contracts = validateContracts(extractJson(res.text));
    if (contracts) {
      return { contracts, provider: res.provider, elapsedMs };
    }
    return {
