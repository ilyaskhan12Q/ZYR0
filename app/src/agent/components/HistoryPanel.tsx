...
export function HistoryPanel({ items, activeId, onSelect }: HistoryPanelProps) {
  return (
    <aside className="hidden w-64 shrink-0 border-l border-border bg-card/40 p-3 md:block">
        ...
        <div className="flex flex-col gap-1">
          {items.map((item) => (
            <button
              ...
              className={`rounded-lg border px-2.5 py-2 text-left text-[11px] transition ${
                activeId === item.id
