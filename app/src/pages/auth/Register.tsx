          ...
          {step === 'role' && (
            <motion.div key="role" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4" role="radiogroup" aria-label="Select register role">
                {roles.map((role) => (
                  <button
                    ...
                    className={`text-left p-6 rounded-xl border-2 transition-all duration-300 focus-visible-ring ${
                  ...
                  >
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-4 ${selectedRole === role.id ? 'bg-accent' : 'bg-accent/10'}`}>
                      <role.icon className={`w-6 h-6 ${selectedRole === role.id ? 'text-white' : 'text-accent'}`} aria-hidden="true" />
                    </div>
                    <h3 className="font-semibold">{role.title}</h3>
                    <p className="text-sm text-muted-foreground mt-1">{role.desc}</p>
                    <ul className="mt-3 space-y-1" aria-label={`${role.title} features`}>
                      {role.features.map((f, i) => (
                        <li key={i} className="flex items-center gap-1.5 text-xs text-muted-foreground">
                          <CheckCircle2 className="w-3 h-3 text-emerald-500" aria-hidden="true" /> {f}
                        </li>
                      ))}
                    </ul>
                  </button>
              ...
              <button
                ...
                className="w-full mt-6 flex items-center justify-center gap-2 bg-accent text-white py-3 rounded-lg font-medium hover:bg-accent/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed focus-visible-ring"
              >
            ...
            <motion.div key="form" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
              <div className="bg-card rounded-xl border border-border shadow-lg p-8">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div>
                    <label htmlFor="reg-name" className="text-sm font-medium mb-1.5 block">Full Name</label>
                  ...
                  <div>
                    <label htmlFor="reg-email" className="text-sm font-medium mb-1.5 block">Email Address</label>
                    <div className="relative">
                  ...
                  <div>
                    <label htmlFor="reg-pass" className="text-sm font-medium mb-1.5 block">Password</label>
                    <div className="relative">
                  ...
                  <div>
                    <label htmlFor="reg-confirm-pass" className="text-sm font-medium mb-1.5 block">Confirm Password</label>
                    <div className="relative">
                      <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" aria-hidden="true" />
                      <input
                        id="reg-confirm-pass"
                  ...
                  <label htmlFor="reg-terms" className="flex items-start gap-2 text-sm cursor-pointer select-none">
                    <input
                      id="reg-terms"
                      type="checkbox"
                      required
                      className="mt-0.5 rounded border-border focus-visible-ring"
                    />
                    <span className="text-muted-foreground">I agree to the <Link to="#" className="text-accent hover:underline focus-visible-ring rounded-sm">Terms of Service</Link> and <Link to="#" className="text-accent hover:underline focus-visible-ring rounded-sm">Privacy Policy</Link></span>
                  </label>
                  <button
                    type="submit"
                    disabled={loading}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button
                    ...
                    className="flex items-center justify-center gap-2 py-2.5 border border-border rounded-lg hover:bg-muted transition-colors text-sm font-medium focus-visible-ring"
                  >
                ...
                <form onSubmit={handleVerifyOtp} className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 border border-border rounded-lg bg-background focus:outline-none focus:ring-2 focus:ring-accent focus:border-transparent text-center font-mono text-2xl tracking-widest placeholder:text-muted-foreground/30 placeholder:tracking-normal placeholder:font-sans focus-visible-ring"
                    />
                  </div>
                  ...
                  <button
                    ...
                    className="w-full py-3 bg-accent text-white rounded-lg font-medium hover:bg-accent/90 focus:outline-none focus:ring-2 focus:ring-accent/50 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 focus-visible-ring"
                  >
                    {loading ? (
                      <span className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" aria-hidden="true" />
                    ) : (
