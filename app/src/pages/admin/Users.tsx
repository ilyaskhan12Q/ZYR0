...
import { Search, Shield, GraduationCap, Building2, UserCheck, Eye, Edit } from 'lucide-react';
...
const roleConfig: Record<string, { color: string; icon: React.ElementType }> = {
  ...
  company: { color: 'bg-emerald-100 text-emerald-700', icon: Building2 },
  mentor: { color: 'bg-purple-100 text-purple-700', icon: UserCheck },
...
export default function AdminUsers() {
  ...
  const filtered = users.filter(u => {
    const matchTab = activeTab === 'All' || u.role === activeTab.toLowerCase();
                  ...
                  <motion.tr key={user.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        ...
                        <button className="p-1.5 hover:bg-muted rounded-lg" title="Edit"><Edit className="w-4 h-4 text-muted-foreground" /></button>
                      </div>
                    </td>
                  </motion.tr>
                );
