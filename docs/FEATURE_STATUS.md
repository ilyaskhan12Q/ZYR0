...
| **Company Team Roles & Portal Access** | ✅ | Email invites with tokenized acceptance (`/accept-invite`), role assignment (`admin/hr/mentor/reviewer`), role-scoped RLS on internships/applications/tasks/certificates/offer-letters/messaging, and three-layer tab gating (nav filter + route guard + DB policies). |
